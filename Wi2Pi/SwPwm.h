...
#define ROUNDUP(X, M)		    ((((X) / (M)) * (M)) + ((X) % (M) > 0 ? (M) : 0))
	...
	private:
		...
		bool CreateAndInitDmaControlBlocks()
		{
			...
			auto res = MemMap.AllocMap(ControlDataLength);
			CtrlDataPA = res.PhysicalAddress;
			CtrlDataVA = (PControlData)res.UserAddress;
			...
			LogInfo("Allocated %d byte ControlData block @VA:0x%08x @PA:0x%08x", ControlDataLength, CtrlDataVA, CtrlDataPA);
	...
	private:
		const ULONG ControlDataLength = ROUNDUP(sizeof(ControlData), 4096);
		ULONG CtrlDataPA;
		PControlData CtrlDataVA;
	};
}